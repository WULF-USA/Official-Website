...
describe WulfApp, :performance do
    it "renders empty home page in under 100 ms" do
        expect { visit "/en/" }.to perform_under(150).ms.and_sample(100)
    end
    
    it "renders empty home page 10 times per second" do
        expect { visit "/en/" }.to perform_at_least(8).ips
    end
    ...
    it "logs in and out as the super user in under 200 ms" do
        expect {
            sso_super_login
            sso_super_logout
        }.to perform_under(300).ms.and_sample(100)
    end
    ...
    it "posts 100 news stories in under 100 ms each iteration" do
        ...
        expect {
            ...
            click_on 'Submit'
        }.to perform_under(150).ms.and_sample(100)
        sso_super_logout
    end
    
    it "loads the news author dashboard in under 100 ms each iteration" do
        sso_super_login
        expect { visit "/en/author/news" }.to perform_under(150).ms.and_sample(100)
        sso_super_logout
    end
    ...
    it "loads the video link author dashboard in under 100 ms each iteration" do
        sso_super_login
        expect { visit "/en/author/videos" }.to perform_under(150).ms.and_sample(30)
        sso_super_logout
    end
    ...
    it "loads the articles author dashboard in under 100 ms each iteration" do
        sso_super_login
        expect { visit "/en/author/articles" }.to perform_under(150).ms.and_sample(100)
        sso_super_logout
    end
    ...
    it "loads the loaded blog in under 100 ms each iteration" do
        expect { visit "/en/blog" }.to perform_under(150).ms.and_sample(100)
    end
    
    it "renders loaded blog 10 times per second" do
        expect { visit "/en/blog" }.to perform_at_least(8).ips
    end
